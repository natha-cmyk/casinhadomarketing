...
export interface HydrationData {
  config: { redes: Record<string, boolean>; paineis: Record<string, Record<string, boolean>>; contas: Record<string, boolean>; cfgOpen: Record<string, boolean>; impOpen: boolean; calManuais?: string[]; calDefaults?: Record<string, string>; agentsConfig?: Record<string, { enabled: boolean; panels: string[] | null; promptExtra: string; name?: string }>; widgetLayout?: Record<string, { order?: string[]; size?: Record<string, number>; height?: Record<string, number>; hidden: string[]; grid?: Record<string, { x: number; y: number; w: number; h: number }> }> } | null;
...
export function saveConfig(s: UIState) {
  return putJSON("/api/config", {
    ...
    calManuais: s.calManuais, calDefaults: s.calDefaults, agentsConfig: s.agentsConfig, widgetLayout: s.widgetLayout,
  });
}
