...
// Resumo LEVE do painel (overview): 1 chamada por conta conectada, montando o
// indicador PRIMÁRIO por plataforma. Uma conta falhar NÃO derruba as outras.
...
//   linkedin                  → linkedin-aggregate (impressões/alcance/…)
//   threads/googlebusiness/…  → só followersCount (sem analytics de conta)
...
async function summarize(a: ZernioAccount, range: { since: string; until: string }): Promise<AccountSummary> {
  ...
  try {
    ...
    // threads / googlebusiness / demais → só followersCount (metrics vazio)
  } catch {
    base.metrics = {};
  }
  return base;
...
export async function GET(req: Request) {
  try {
    ...
    // ads-only (enabled:false, sem analytics) fica fora do overview de canais.
    const connected = accounts.filter(
      (a) => a.enabled === true || (a as Record<string, unknown>).analyticsEnabled === true
    );
